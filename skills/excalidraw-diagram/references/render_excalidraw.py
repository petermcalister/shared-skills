...
def compute_bounding_box(elements: list[dict]) -> tuple[float, float, float, float]:
    ...
    return (0, 0, 800, 600) if min_x == float("inf") else (min_x, min_y, max_x, max_y)
...
def render(
    ...
    with sync_playwright() as p:
        ...
        page = browser.new_page(viewport={"width": vp_width, "height": vp_height}, device_scale_factor=scale)
        page.goto(template_path.as_uri())
        page.wait_for_function("window.__moduleReady === true", timeout=30000)

        result = page.evaluate(f"window.renderDiagram({json.dumps(data)})")
        if not result or not result.get("success"):
            error_msg = result.get("error", "Unknown") if result else "renderDiagram returned null"
