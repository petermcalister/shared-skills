...
Rules for writing **slide content** (titles, bullets, captions, short body fragments). Apply these when generating text for the JSON manifest.

> **For long-form prose** (Option 5 docx, Option 8 Design Doc / RFC, future blog / journey posts), also load `references/generation/tech-writing.md` — it covers document structure, paragraph rhythm (ABT, alternating rhythm), opening hooks, conclusion length, and layered-depth tone for mixed-seniority readers. The slide-shaped rules below (max 5 bullets, max 3-sentence body blocks, fragment bullets) do **not** apply to flowing prose; load `tech-writing.md` as the structural editor in those cases and use this file only for in-line voice (active voice, no hedging, no AI-sounding fluff).
